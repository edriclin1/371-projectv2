...
class Rest
{
    ...
    public function readVersion($access_token) 
    {
        $constants = new Constants($this->clientURL);
        //$user = new User();

        $request = new HTTP_Request2($constants->HOSTNAME . '/learn/api/public/v1/system/version', HTTP_Request2::METHOD_GET);
        $request->setHeader('Authorization', 'Bearer ' . $access_token);
        $request->setConfig(array(
            'ssl_verify_peer' => $constants->ssl_verify_peer,
            'ssl_verify_host' => $constants->ssl_verify_host
        ...
        try {
            ...
            if (200 == $response->getStatus()) {
                //print "\n Read User...\n";
                $learn = json_decode($response->getBody());
            } else {
                print 'Unexpected HTTP status: ' . $response->getStatus() . ' ' . $response->getReasonPhrase();
                $BbRestException = json_decode($response->getBody());
                var_dump($BbRestException);
